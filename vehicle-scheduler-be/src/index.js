...
const server = app.listen(config.port, config.host, async () => {
  ...
  await logger.info('route', `vehicle scheduler started on ${config.host}:${boundPort}`, 'backend').catch(() => {});
  process.stdout.write(`Vehicle Scheduler Backend listening on http://${config.host}:${boundPort}\n`);
});
